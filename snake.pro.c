...
void ShowUI() {
    ...
    coord.X=lx;
    coord.Y=ly;
    // 检查是否是边框位置，如果是则重新绘制边框字符而不是清除
    if((coord.Y == 0 || coord.Y == HIGH) || (coord.X == 0 || coord.X == WIDE + INFO_WIDTH)) {
        SetConsoleCursorPosition(GetStdHandle(STD_OUTPUT_HANDLE), coord);
        SetColor(11); // 青色边框
        if((coord.Y == 0 && coord.X == 0) || (coord.Y == 0 && coord.X == WIDE + INFO_WIDTH) ||
           (coord.Y == HIGH && coord.X == 0) || (coord.Y == HIGH && coord.X == WIDE + INFO_WIDTH)) {
            putchar('+'); // 角落
        } else if(coord.Y == 0 || coord.Y == HIGH) {
            putchar('-'); // 上下边框
        } else {
            putchar('|'); // 左右边框
        }
...
void Playgame() {
    ...
    while(snake.body[0].X > 0 && snake.body[0].X < WIDE && snake.body[0].Y > 0 && snake.body[0].Y < HIGH) {
        ...
        // 根据移动方向调整延迟，平衡上下和左右移动的速度感觉
        // 上下移动时增加额外延迟，因为控制台中字符高度通常大于宽度
        if(dy != 0) { // 上下移动
            Sleep(delay + 50); // 增加50毫秒延迟
        } else { // 左右移动
            Sleep(delay); // 保持原延迟
        }
    }
